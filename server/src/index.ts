...
const model =
  chatProvider === "groq" ? groqModel : openAIModel;
...
function getOpenAIChatClient() {
  ...
  if (!hasRealKey(apiKey, "sk-your-key-here")) {
    throw new Error(
      "OPENAI_API_KEY is missing. Add an OpenAI key or use CHAT_PROVIDER=groq.",
    );
  }
...
function getOpenAIImageClient() {
  ...
  if (!apiKey) {
    throw new Error(
      "Image generation needs an OpenAI API key. Add OPENAI_IMAGE_API_KEY in Vercel.",
    );
  }
...
app.get("/app-config", (_request, response) => {
  const whatsappUrl =
    process.env.CONTACT_WHATSAPP_URL?.trim();

  response.json({
    whatsappUrl: whatsappUrl?.startsWith("https://wa.me/")
      ? whatsappUrl
      : null,
  });
});
    ...
    try {
      ...
      if (chatProvider === "groq") {
        ...
        const result = await openai.responses.create({
          ...
          input: parsed.data.messages.map(
            ({ role, content }) => ({
              role,
              content,
            }),
  ...
  async (request, response, next) => {
    try {
      ...
      if (!base64) {
        response.status(502).json({
          error:
            "The image model returned no image. Please try again.",
        });
        return;
      }
...
if (process.env.VERCEL !== "1") {
  app.listen(port, "0.0.0.0", () => {
    console.log(
      `Nexa AI server running on http://localhost:${port}`,
    );
